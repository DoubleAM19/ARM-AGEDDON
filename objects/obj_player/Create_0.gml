...
// HORIZONTAL STATES
	...
	horizstate_normal = function()
	{
		...
		jump_power = normal_jumppower; // set jumpheight
		...
		if (right - left) == 0 {
			...
			if (current_move_speed > 0) {current_move_speed -= player_friction;}
			if (current_move_speed < 0) {current_move_speed += player_friction;}
		}
	...
	horizstate_dashroll = function()
	{
		...
		jump_power = dashroll_jumppower; // set jumpheight
	...
	// NOINPUT - same as normal but doesnt get influenced by left or right
	horizstate_noinput = function()
	{
		jump_power = normal_jumppower; // set jumpheight
